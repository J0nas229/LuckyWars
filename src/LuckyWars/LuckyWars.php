...
class LuckyWars extends PluginBase implements Listener {
	...
	{
		  $this->getLogger()->info(TE::DARK_AQUA . "LuckyWars by KaitoDoDo");
				...
				if($player->isOp())
				{
					...
					else
					{
					 $player->sendMessage($this->prefix . "LuckyWars Commands!");
                                         $player->sendMessage($this->prefix . "/lw make [world]: Create a lw game!");
                                         $player->sendMessage($this->prefix . "/lwstart: start the game");
					}
				}
			return true;
